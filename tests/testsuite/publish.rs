...
fn delay_publish_underscore() {
    ...
    let _registry = registry::RegistryBuilder::new()
        .http_index()
        .http_api()
        .add_responder("/index/de/la/delay_publish", move |req, server| {
            let mut lock = arc.lock().unwrap();
            *lock += 1;
    ...
    // The sparse-registry test server does not know how to publish on its own.
    // So let us call publish for it.
    Package::new("delay_publish", "0.0.1")
        .file("src/lib.rs", "")
        .publish();
    ...
    p.cargo("build -Z sparse-registry")
        ...
        .with_stderr(
            "\
[UPDATING] [..]
[ERROR] no matching package named `delay_publish` found
location searched: registry `crates-io`
required by package `foo v0.0.1 ([..]/foo)`
",
        )
